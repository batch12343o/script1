-- Ultimate Grow a Garden Pet Dupe Script v7.0
-- Raw URL (hypothetical): https://raw.githubusercontent.com/AnonUser123/GrowAGardenDupeScript/main/pet-dupe-v7.lua
...
Title.Text = "Ultimate Pet Dupe - Grow a Garden v7.0"
...
PetInfo.Size = UDim2.new(0.9, 0, 0.25, 0)
...
DupeButton.Position = UDim2.new(0.05, 0, 0.45, 0)
...
MultiDupeButton.Position = UDim2.new(0.36, 0, 0.45, 0)
...
DebugButton.Position = UDim2.new(0.67, 0, 0.45, 0)
...
local maxAttempts = 5
local possibleEvents = {"DupePet", "PetDupe", "ClonePet", "ReplicatePet", "EquipPet", "PetEvent"} -- Expanded event names
...
local function findHeldPet()
    for _, path in pairs(possibleEquippedPaths) do
        ...
        if current and (current:IsA("StringValue") or current:IsA("ObjectValue")) then
            ...
            if pet and pet:IsDescendantOf(LocalPlayer:FindFirstChild("PetInventory")) then
                return pet
            end
        end
    end
    ...
    if petInventory and #petInventory:GetChildren() > 0 then
        return petInventory:GetChildren()[1] -- Fallback to first pet
    end
    return nil
end
...
local function updatePetInfo()
    ...
    if heldPet then
        local weight = heldPet:FindFirstChild("Weight") and heldPet.Weight.Value or
                       heldPet:FindFirstChild("PetWeight") and heldPet.PetWeight.Value or "Unknown"
        local age = heldPet:FindFirstChild("Age") and heldPet.Age.Value or
                    heldPet:FindFirstChild("PetAge") and heldPet.PetAge.Value or "Unknown"
        local petId = heldPet:FindFirstChild("Value") and heldPet.Value or heldPet.Name
...
local function debugPetData()
    ...
    while parent and parent ~= game do
        debugInfo = debugInfo .. "Parent: " .. parent.Name .. "\n"
...
local function dupePet()
    ...
    local success, err = pcall(function()
        local args = {
            ...
            [3] = heldPet:FindFirstChild("Value") and heldPet.Value or heldPet.Name
        }
...
local function multiDupe()
    ...
    for i = 1, maxAttempts do
        ...
        wait(math.random(0.1, 0.2)) -- Randomized delay
    end
end
...
-- Initial Setup
updatePetInfo()
print("Ultimate Grow a Garden Pet Dupe Script v7.0 Loaded")
